(function (root) {
  ...
  async function cacheProcessedUrlForFile(file) {
    ...
    try {
      ...
      console.warn("Twinmap icon prepare failed:", file, error);
    }
  ...
  function toMapboxImage(image) {
    try {
      ...
      console.warn("Twinmap icon raster fallback:", error);
      return image;
    }
  }
  ...
  function loadMapIcons(map, options) {
    ...
    map.__twinmapIconsReady = (async () => {
      ...
      for (const file of ALL_ICON_FILES) {
        ...
        if (!image) {
          console.warn("Twinmap icon load failed:", file, url);
          continue;
        }
        ...
        try {
          ...
          try {
            ...
            console.warn("Twinmap icon add failed:", file, addError, rawError);
          }
        }
      }
      ...
      console.info(`TwinMap: ${loaded}/${ALL_ICON_FILES.length} iconos de categoría cargados`);
      return loaded;
    })();
